        ...
        <div className="flex mt-2 mb-2 gap-1">
          ...
          <button
            ...
            onClick={() => {
              ...
              //   res.info.cuisines.filter((cuisine)=>cuisine.toLowerCase()===searchText.toLowerCase())
              // );
              //Above one doesn't work because filter returns an array and array is always truthy
              //so it will give all restaurants but some will return boolean value and filter will
              //take that boolean value give only the restaurant which include this.
              // Why .some() Works Better
              // .filter() on cuisines returns an array .some() returns true / false
              // Outer restaurant .filter() needs a boolean
              const filteredRes = listOfRestaurants.filter((res) =>{
                const matchedNames = res.info.name.toLowerCase().includes(searchText.toLowerCase());
                const matchedCuisines = res.info.cuisines.some((cuisine)=>cuisine.toLowerCase().includes(searchText.toLowerCase()));
                return (matchedNames || matchedCuisines);
            });
